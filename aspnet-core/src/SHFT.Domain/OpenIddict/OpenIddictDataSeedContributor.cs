...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("SHFT") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "SHFT", DisplayName = "SHFT API", Resources = { "SHFT" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["SHFT_Swagger:ClientId"];

        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["SHFT_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        var webClientId = configurationSection["SHFT_Web:ClientId"];

        if(!webClientId.IsNullOrWhiteSpace())
        {
            var webRootUrl = configurationSection["SHFT_Web:RootUrl"];
            var swaggerRootUrl = configurationSection["SHFT_Swagger:RootUrl"]?.TrimEnd('/');
